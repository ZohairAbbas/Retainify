...
const DEFAULT_BRAND = {
  ...
  bg: '#FFFFFF',
  fontPair: 'editorial', // editorial | modern | classic
};
    ...
    if (block.placeholder) {
      return (
        <div className="rt-emb-image-placeholder" style={{ height: block.height }}>
          <Icons.Image size={20} />
          <span className="t-mono" style={{ fontSize: 11, marginTop: 6 }}>{block.label || 'Image'}</span>
          <span className="t-mono faint" style={{ fontSize: 10, marginTop: 2 }}>{block.align === 'full' ? '600 × ' + block.height : ''}</span>
        </div>
      );
    }
  ...
  if (block.type === 'discount') {
    return (
      <div className="rt-emb-discount" style={{ borderColor: brand.accent }}>
        ...
        <div className="rt-emb-discount-code" style={{ fontFamily: 'Geist Mono, ui-monospace, monospace', color: '#14201A' }}>{block.code}</div>
...
Object.assign(window, { EmailEditor, defaultBlocks: defaultBlocks, FONT_PAIRS, DEFAULT_BRAND });
