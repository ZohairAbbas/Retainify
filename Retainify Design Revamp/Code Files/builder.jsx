      ...
      <div className="rt-ins">
        <div className="rt-ins-head">
          ...
          <div>
            ...
            <div className="t-h2" style={{ fontFamily: 'var(--font-display)', fontWeight: 400 }}>{trig.label}</div>
          </div>
        </div>
        <div className="rt-ins-section">
          <div className="t-micro muted">Trigger event</div>
          <select className="select" value={node.trigger} onChange={e => onChange({ nodePatch: { trigger: e.target.value }})} style={{ marginTop: 8 }}>
            {Object.values(TRIGGERS).map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
          </select>
          <div className="field-help">{trig.desc}</div>
        </div>
...
// ── Builder main ──────────────────────────────────────────────────────────
function Builder({ flow, onBack, onUpdate }) {
  ...
  const handleInspectorChange = ({ flowName, nodePatch }) => {
