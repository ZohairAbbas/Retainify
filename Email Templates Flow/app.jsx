...
const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{
  ...
  "showSegmentsEmpty": false,
  "segmentsLanding": "list"
}/*EDITMODE-END*/;
...
function App() {
  ...
  const [screen, setScreen] = useStateApp('list'); // 'list' | 'builder' | 'email' | 'popups' | 'popup-editor' | 'contact-profile' | 'segments-list' | 'segments-builder' | 'segments-detail'
  const [activeContactId, setActiveContactId] = useStateApp(null);
  const [activeSegment, setActiveSegment] = useStateApp(null); // full segment object
  const [builderSegment, setBuilderSegment] = useStateApp(null); // segment being edited in builder (null = new)
  const [userSegments, setUserSegments] = useStateApp(() => JSON.parse(JSON.stringify(window.RetainifySegments.USER_SEGMENTS)));
  ...
  const handleNav = (id) => {
    ...
    setBuilderSegment(null);
    if (id === 'flows') { setScreen('list'); }
    else if (id === 'popups') { setScreen('popups'); }
    else if (id === 'contacts') { setScreen('list'); }
    else if (id === 'segments') { setScreen('segments-list'); }
    else { setScreen('list'); /* other nav items out of scope */ }
  };

  // ── Segment handlers ──
  const handleOpenSegmentBuilder = (segment) => { setBuilderSegment(segment); setScreen('segments-builder'); };
  const handleOpenSegmentDetail = (segment) => { setActiveSegment(segment); setScreen('segments-detail'); };
  const handleBackToSegments = () => { setActiveSegment(null); setBuilderSegment(null); setScreen('segments-list'); };
  const handleSaveSegment = (payload) => {
    if (payload.id) {
      setUserSegments(prev => prev.map(s => s.id === payload.id ? { ...s, ...payload, updated: 'just now' } : s));
    } else {
      const newSeg = {
        ...payload,
        id: 'sg_' + Math.random().toString(36).slice(2, 7),
        contactCount: 142 + Math.floor(Math.random() * 800),
        delta: '+0 this week', deltaPct: 0,
        updated: 'just now',
        flows: [],
        sparkline: [120, 122, 124, 126, 128, 130, 134, 138, 140, 141, 142],
      };
      setUserSegments(prev => [newSeg, ...prev]);
      setActiveSegment(newSeg);
      setScreen('segments-detail');
      return;
    }
    setBuilderSegment(null);
    setScreen('segments-list');
  };
  const handlePickTemplate = (template) => {
    ...
    setScreen('segments-builder');
  };
  const handleUseInFlow = (segment) => {
    // Drop the user into a blank flow with this segment pre-filled as the trigger.
    const triggerNode = { id: 'trig', type: 'trigger', trigger: 'entered_segment', segmentId: segment.id, segmentName: segment.name, frequency: 'none' };
    const exitNode = { id: 'exit', type: 'exit' };
    const newFlow = {
      id: 'f_' + Math.random().toString(36).slice(2, 7),
      name: `${segment.name} flow`,
      status: 'draft',
      trigger: 'entered_segment',
      updated: 'just now',
      sent: null, openRate: null, clickRate: null,
    ...
    <>
      <ShopifyAppShell active={navActive} onNav={handleNav}>
        {navActive === 'flows' ? (
          ...
          screen === 'segments-builder' ? (
            <SegmentBuilder
              initial={builderSegment}
              onBack={handleBackToSegments}
              onSave={handleSaveSegment}
              onSaveAndUseInFlow={(payload) => { handleSaveSegment(payload); setShowCreate(true); setNavActive('flows'); setScreen('list'); }}
            />
          ) : screen === 'segments-detail' && activeSegment ? (
            <SegmentDetail
              segment={activeSegment}
              onBack={handleBackToSegments}
              onEdit={handleOpenSegmentBuilder}
              onUseInFlow={handleUseInFlow}
            />
          ) : (
            <SegmentsList
              showEmpty={tweaks.showSegmentsEmpty}
              onOpenBuilder={handleOpenSegmentBuilder}
              onOpenDetail={handleOpenSegmentDetail}
  ...
  return (
    <div className="rt-tweaks">
      ...
      <div className="rt-tweaks-body">
        ...
        <div className="rt-tweak">
          <label className="rt-toggle">
            <input type="checkbox" checked={tweaks.showSegmentsEmpty} onChange={e => setTweak('showSegmentsEmpty', e.target.checked)} />
            <span className="rt-toggle-switch" />
            <span>Show empty state (Segments)</span>
          </label>
        </div>
