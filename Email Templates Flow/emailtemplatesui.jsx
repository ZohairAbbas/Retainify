// Retainify — Email Template Gallery UI
// Full-screen takeover for browsing email templates, grouped by vibe.
// Opens from the email editor's "Browse templates" top-bar button.

const { useState: useStateETU, useEffect: useEffectETU, useMemo: useMemoETU, useLayoutEffect: useLayoutEffectETU, useRef: useRefETU } = React;
...
function FitEmail({ children, naturalWidth, naturalHeight, fillFactor = 0.96, align = 'top' }) {
  const wrapRef = useRefETU(null);
  const [scale, setScale] = useStateETU(0.4);
  useLayoutEffectETU(() => {
...
// ── Static email render (uses window.BlockView) ─────────────────────────
function RenderTemplateEmail({ template, width = 600 }) {
  const BlockView = window.BlockView;
  const blocks = template.blocks;
  const brand = template.brand;
  return (
    <div className="rt-emt-paper" style={{ width, background: brand.bg }}>
      <div style={{ padding: '0 28px' }}>
        {blocks.map(b => (
          <div key={b.id} className="rt-emt-block">
            <BlockView block={b} brand={brand} isPreview={true} />
          </div>
        ))}
      </div>
    </div>
...
function EmailTemplateCard({ template, onSelect, onUseTemplate }) {
  const journey = window.EmailTemplates.JOURNEYS[template.journey];
  const TrigIcon = window.Icons[journey.icon];
  return (
      ...
      <div className="rt-emt-card-meta">
        ...
        <div className="rt-emt-card-foot">
          ...
          <button className="btn btn-primary btn-sm" onClick={(e) => { e.stopPropagation(); onUseTemplate(template.id); }}>
            Use design <Icons.Arrow size={11} />
          </button>
        </div>
      </div>
    </article>
    ...
    <div className="rt-emt-confirm-backdrop" onClick={onCancel}>
      <div className="rt-emt-confirm-modal" onClick={e => e.stopPropagation()}>
        ...
        <p className="t-body muted" style={{ margin: 0, lineHeight: 1.5, maxWidth: 380 }}>
          Applying <strong style={{ color: 'var(--ink-1)' }}>{template.name}</strong> will overwrite your current blocks, brand kit (fonts, colors, logo), subject line and preview text.
        </p>
        ...
        <div className="rt-emt-confirm-actions">
          ...
          <button className="btn btn-primary" onClick={onConfirm}>
            Replace with {template.name} <Icons.Arrow size={12} />
          </button>
        </div>
      </div>
    </div>
...
function EmailTemplateGallery({ onClose, onUseTemplate, currentTemplateId }) {
  const { VIBES, VIBE_ORDER, JOURNEYS, TEMPLATES, TEMPLATE_ORDER } = window.EmailTemplates;
  const [activeFilter, setActiveFilter] = useStateETU('all');
  const [previewId, setPreviewId] = useStateETU(null);
  const [pendingId, setPendingId] = useStateETU(null);
  const sectionRefs = useRefETU({});

  const visibleVibes = activeFilter === 'all' ? VIBE_ORDER : [activeFilter];

  const templatesByVibe = useMemoETU(() => {
    const map = {};
    ...
    <div className="rt-emt-shell">
      ...
      <div className="rt-emt-body">
        ...
        <aside className="rt-emt-rail">
          ...
          >
            <span className="rt-emt-rail-glyph"><Icons.Layers size={13} /></span>
        ...
        <main className="rt-emt-main">
          ...
          {visibleVibes.map(vId => {
            ...
            return (
              <section
                key={vId}
                className="rt-emt-section"
                ref={el => sectionRefs.current[vId] = el}
              >
...
function EmailTemplatePreview({ template, onUseTemplate, onClose }) {
  const journey = window.EmailTemplates.JOURNEYS[template.journey];
  const TrigIcon = window.Icons[journey.icon];
  const [viewport, setViewport] = useStateETU('desktop');
        ...
        <div className="rt-emt-modal-preview">
          <div className="rt-emt-modal-preview-bar">
            <div className="rt-emt-modal-preview-inbox">
              <div className="rt-emt-modal-preview-from">{template.brandPersona} <span className="muted">· hello@{template.brandPersona.toLowerCase().replace(/[^a-z0-9]/g, '')}.co</span></div>
            ...
            <div className="rt-pop-seg" style={{ width: 160 }}>
              <button className={viewport === 'desktop' ? 'rt-on' : ''} onClick={() => setViewport('desktop')}>
                <Icons.Desktop size={12} /> Desktop
              </button>
              <button className={viewport === 'mobile' ? 'rt-on' : ''} onClick={() => setViewport('mobile')}>
        ...
        <aside className="rt-emt-modal-detail">
          ...
          <div className="rt-emt-modal-detail-inner">
              ...
              {template.discount > 0 && (
                <li>
                  ...
                  <div>
                    ...
                    <div className="t-small muted">Code auto-generates per recipient.</div>
                  </div>
                </li>
              )}
              <li>
            ...
            <button className="btn btn-primary btn-lg" style={{ width: '100%', marginTop: 28 }} onClick={onUseTemplate}>
              Use this design <Icons.Arrow size={14} />
            </button>
            <button className="btn btn-ghost" style={{ width: '100%', marginTop: 8 }}>
              <Icons.Send size={12} /> Send a sample to me
            </button>
          </div>
        </aside>
      </div>
...
window.EmailTemplateComponents = { EmailTemplateGallery, EmailTemplatePreview, RenderTemplateEmail, ReplaceConfirmModal };
